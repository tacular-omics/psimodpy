...
if missing). .zenodo.json must not carry a version (Zenodo takes it from
...
def set_date_released(citation: str, day: str) -> str:
    ...
    updated, count = re.subn(r"^(version: *[^\n]+)$", lambda match: f"{match[1]}\ndate-released: {json.dumps(day)}", citation, flags=re.MULTILINE)
    if count != 1:
        raise ValueError("CITATION.cff needs exactly one version field")
    return updated
