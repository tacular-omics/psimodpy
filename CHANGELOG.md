...
- `search_mass(delta, *, tolerance=0.01, unit="da", site=None, position=None) -> list[tuple[entry, float]]`: entries whose `diff_mono` (monoisotopic mass difference) is within `tolerance` of `delta`, as `(entry, error)` pairs with `error = delta - mass`, closest first (ties in mass, then file order). `unit` is `"da"` or `"ppm"` (ppm of `|delta|`). Entries without a mass are skipped. `site` is one or more residue letters (`"S"`, `"STY"`, any case) or `"N-term"`/`"C-term"`, matched against the entry's origin residues (`origin`; crosslinks list each residue). `position` is where the residue was observed: `"anywhere"`, `"peptide n-term"`, `"peptide c-term"`, `"protein n-term"`, `"protein c-term"` (case-insensitive; `"Any N-term"`/`"Any C-term"` are aliases for the peptide ones); it is matched against PSI-MOD's N-term/C-term flag (`term_spec`), which does not say protein or peptide: an N-terminal mod matches both `"peptide n-term"` and `"protein n-term"`. Entries with no flag count as anywhere. A modification of the terminus itself matches any residue at that terminus. A sorted mass index is built on the first call, and each search is a bisect. A bad `site`, `position`, `unit`, `delta` or `tolerance` raises `PsimodError` (a `ValueError`).
- `get_by_site(site)`: the same as `get_by_origin(site.strip().upper())` (kept), named to match unimodpy and uniprotptmpy, with each entry listed once (a `"S, S"` crosslink appears twice in `get_by_origin("S")`). Unknown or non-string input returns `[]`.
